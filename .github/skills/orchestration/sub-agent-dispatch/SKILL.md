...
> Dev/Tester read `plan.md` as **intent + binding decisions, not a checklist** (see
> `orchestration/spec-ingest`). Honour clarified technical decisions; they own their own
> implementation/test order. Tests target behaviour from `spec.md`, not internal plan detail.

### Phase 2 — DeveloperAgent
```
Task: Implement .NET code from the active feature spec
Read: specs/<NNN-feature>/spec.md + plan.md (+ data-model.md, contracts/),
      .specify/memory/constitution.md, .github/copilot-instructions.md
...
```
Task: Write tests from the active feature spec
Read: specs/<NNN-feature>/spec.md + plan.md, .specify/memory/constitution.md
...
```
Task: Review the change against the constitution + conventions + .NET best practices
Read: git diff (merge-base) / src/, specs/<NNN-feature>/spec.md + plan.md,
      .specify/memory/constitution.md, .github/copilot-instructions.md
