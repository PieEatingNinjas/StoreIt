...
- The active feature's spec exists under `specs/<NNN-feature>/` (Spec Kit output: `spec.md`, `plan.md`).
- The constitution exists at `.specify/memory/constitution.md`.
- .NET SDK + solution (`src/<Solution>.sln`) present.

## Phase 0 — Context
1. Read the active feature's `spec.md` + `plan.md` (+ `data-model.md`, `contracts/`) fully.
2. Read `.specify/memory/constitution.md` — the invariants your code must honour.
3. Read all relevant contracts/data-model before writing code.
